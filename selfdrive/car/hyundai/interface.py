...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=[]):  # pylint: disable=dangerous-default-value
    ...
    ret.longitudinalTuning.kpV = [1.0, 1.2, 0.75, 0.65] # 브레이크 포인트에 해당하는 비례값, 그 사이는 보간값임. 값이 클수록 해당속도에 빨리 도달(가스/브레이크 둘다 영향)
    ret.longitudinalTuning.kiBP = [0., 15., 35.] # 브레이크 포인트(속도, 단위:m/s)
    ret.longitudinalTuning.kiV = [0.3, 0.2, 0.15] # 적분값, 비례값에 대한 오차를 보상함. 도달속도가 느릴수록 값을 올려야함. 오버슈트가 날경우 값을 줄여야함.
